...
class _TactileDeckAppState extends State<TactileDeckApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Mylestography Control Studio',
...
class _ControlDeckScreenState extends State<ControlDeckScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Milestone 2: warn only above 80%; exactly 80% stays normal.
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Milestone 1: four camera controls use distinct icons, labels and accents.
            Wrap(
              spacing: 20,
              runSpacing: 20,
              ...
              children: [
                TactileButton(
                  ...
                  sharedPressed: sharedPressed,
                  onPressedChanged: (value) =>
                      setState(() => sharedPressed = value),
                  onPressed: () => _triggerAction("PHOTO CAPTURE"),
                ),
                TactileButton(
                  ...
                  onPressedChanged: (value) =>
                      setState(() => sharedPressed = value),
                  onPressed: () => _triggerAction("FOCUS LOCK"),
                ),
                TactileButton(
                  ...
                  onPressedChanged: (value) =>
                      setState(() => sharedPressed = value),
                  onPressed: () => _triggerAction("BURST CAPTURE"),
                ),
                TactileButton(
                  icon: Icons.flash_on,
                  label: "FLASH",
                  ...
                  onPressedChanged: (value) =>
                      setState(() => sharedPressed = value),
                  onPressed: () => _triggerAction("FLASH TEST"),
                ),
              ],
            ),
            const SizedBox(height: 24),
            // Extra credit: hold to recharge; a normal tap explains the gesture.
            TactileButton(
              ...
              onPressedChanged: (value) =>
                  setState(() => sharedPressed = value),
              onPressed: () =>
                  setState(() => systemStatus = 'HOLD TO RECHARGE'),
              onLongPress: () => setState(() {
                powerLevel = 100;
                totalTaps++;
                systemStatus = 'BATTERY RECHARGED';
              }),
            ),
            const SizedBox(height: 20),
            Text(
              isOverload ? 'HIGH POWER — above 80%' : 'POWER IN NORMAL RANGE',
            ),
...
class _TactileButtonState extends State<TactileButton> {
  ...
  bool get isPressed => widget.sharedPressed;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTapDown: (_) {
        // Evidence for Question 2: log the exact gesture callback order.
        debugPrint('${widget.label}: onTapDown');
        widget.onPressedChanged(true);
      },
      // 2. User releases button -> restore position and fire callback
      onTapUp: (_) {
        debugPrint('${widget.label}: onTapUp');
        widget.onPressedChanged(false);
        widget.onPressed();
      },
      // 3. User cancels touch -> restore position safely
      onTapCancel: () {
        debugPrint('${widget.label}: onTapCancel');
        widget.onPressedChanged(false);
      },
